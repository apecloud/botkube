...
type (
	...
	// MessageOption defines option mutator signature.
	MessageOption func(options *MessageOptions)
...
// WithAdditionalSelects adds additional selects to a given kubectl KbcliCmdBuilderMessage message.
func WithAdditionalSelects(in ...*api.Select) MessageOption {
...
// WithAdditionalSections adds additional sections to a given kubectl KbcliCmdBuilderMessage message.
func WithAdditionalSections(in ...api.Section) MessageOption {
	return func(options *MessageOptions) {
		options.sections = append(options.sections, in...)
	}
}

// KbcliCmdBuilderMessage returns message for constructing kubectl command.
func KbcliCmdBuilderMessage(dropdownsBlockID string, cmds api.Select, opts ...MessageOption) api.Message {
...
func FilterSection() api.LabelInput {
	return api.LabelInput{
		...
		Command: fmt.Sprintf("%s %s %s ", api.MessageBotNamePlaceholder, kbcliCommandName, filterPlaintextInputCommand),
	}
}

// CmdSelect return drop-down select for kbcli commands.
func CmdSelect(cmds []string, initialItem string) *api.Select {
	return selectDropdown("Select command", cmdsDropdownCommand, dropdownItemsFromSlice(cmds), newDropdownItem(initialItem, initialItem))
}

// VerbSelect return drop-down select for kubectl command verbs.
func VerbSelect(verbs []string, initialItem string) *api.Select {
	return selectDropdown("Select verb", verbsDropdownCommand, dropdownItemsFromSlice(verbs), newDropdownItem(initialItem, initialItem))
}

// ResourceNamesSelect return drop-down select for kbcli resources names.
...
// ResourceNamespaceSelect return drop-down select for kbcli allowed namespaces.
...
func selectDropdown(name, cmd string, items []dropdownItem, initialItem dropdownItem) *api.Select {
	...
	return &api.Select{
		Name:          name,
		Command:       fmt.Sprintf("%s %s %s", api.MessageBotNamePlaceholder, kbcliCommandName, cmd),
